...
public class MyUsernamePasswordAuthProvider extends
	...
	protected com.feth.play.module.pa.providers.password.UsernamePasswordAuthProvider.SignupResult signupUser(final MyUsernamePasswordAuthUser user) {
		...
		if (u != null) {
			if (u.emailValidated) {
				// This user exists and is active, and has its email validated.
				return SignupResult.USER_EXISTS;
			} else {
				// This user exists and is active, but has not yet validated its email.
		...
		// Usually the email should be verified before allowing login; however if you return return SignupResult.USER_CREATED, then the user gets logged in directly.
				...
				for (final LinkedAccount acc : u.linkedAccounts) {
					if (getKey().equals(acc.providerKey)) {
						if (authUser.checkPassword(acc.providerUserId, authUser.getPassword())) {
							...
							// If you don't return here, you would allow the user to have multiple passwords defined. Usually we don't want this.
